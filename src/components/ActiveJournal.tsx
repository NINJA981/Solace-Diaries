...
export default function ActiveJournal({ token, userApiKey, customPrompt, activeEntry, onSaveSuccess, onCancel, isFocusMode, setIsFocusMode }: ActiveJournalProps) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('title', title || 'Untitled Entry');
    ...
    <div className="max-w-5xl mx-auto py-8 px-4 font-sans text-[#E7E7EC] relative z-10">
        ...
        {!isFocusMode && (
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 bg-white/[0.03] border border-white/10 rounded-[2px] flex items-center justify-center text-[var(--accent-color)]">
              ...
              <div>
                ...
                <p className="text-xs text-[#ADA9BA] leading-relaxed">Write down your thoughts. The app will automatically analyze tags and emotions.</p>
              </div>
            </div>
            <button
              onClick={onCancel}
              className="text-xs font-bold text-[#ADA9BA] hover:text-[#F3F3F5] bg-white/[0.04] hover:bg-white/[0.08] px-4 py-2.5 rounded-[2px] transition-all duration-200 cursor-pointer"
            >
              Cancel
            </button>
          </motion.div>
      ...
      <AnimatePresence>
        {pendingPrompt && !activeEntry && !savedData && !isFocusMode && (
          <motion.div 
            ...
            className="mb-6 bg-gradient-to-r from-[var(--accent-gradient-from)]/5 via-[var(--accent-heart)]/5 to-transparent border border-[var(--accent-color)]/20 rounded-[2px] p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 relative overflow-hidden backdrop-blur-md"
          >
            ...
            <div className="flex gap-3">
              <Sparkles className="w-5 h-5 text-[var(--accent-color)] shrink-0 mt-0.5" />
              <div>
            ...
            <button
              ...
              className="text-xs bg-white/[0.04] border border-white/10 hover:border-[var(--accent-color)] text-[#E7E7EC] px-4 py-2 rounded-[2px] transition-all duration-200 font-semibold shrink-0"
            >
              Respond
            </button>
          </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 transition-all duration-500">
        ...
        <motion.form 
          layout
          onSubmit={handleSave} 
          className={`glass-card rounded-[2px] p-6 md:p-10 shadow-2xl space-y-6 relative overflow-hidden transition-all duration-700 ${isFocusMode ? 'lg:col-span-3 max-w-4xl mx-auto w-full min-h-[80vh] border-transparent bg-transparent shadow-none backdrop-blur-none' : 'lg:col-span-2'}`}
        >
          {error && (
            <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-[2px] flex items-start gap-3 text-rose-300 text-xs animate-fade-in">
          ...
          <div className="flex items-center justify-between border-b border-white/5 pb-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-[2px] text-xs font-semibold bg-white/[0.04] hover:bg-white/[0.08] border border-white/5 hover:border-white/10 text-[#ADA9BA] hover:text-[#F3F3F5] transition cursor-pointer select-none"
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-[2px] text-xs font-semibold transition border cursor-pointer select-none ${
                  isRecording
                    ? 'bg-rose-500/10 border-rose-500/30 text-rose-300 animate-pulse ring-2 ring-rose-500/10'
                    : speechSupported
                    ? 'bg-white/[0.04] border-white/5 hover:border-[var(--accent-color)] text-[#ADA9BA] hover:text-[#F3F3F5]'
              ...
              {/* Attach Images */}
              <label
                className="flex items-center gap-2 px-3 py-1.5 rounded-[2px] text-xs font-semibold bg-white/[0.04] border border-white/5 hover:border-[var(--accent-color)] text-[#ADA9BA] hover:text-[#F3F3F5] transition cursor-pointer select-none"
          ...
          <div className="space-y-1">
            <input
              ...
              className="w-full bg-transparent text-xl md:text-2xl font-serif font-bold text-[#F3F3F5] placeholder-[#ADA9BA]/30 border-b border-white/5 py-2 outline-none transition focus:border-[var(--accent-color)]"
            />
          </div>
            ...
            <div className="space-y-3.5 pt-4 border-t border-white/5 animate-fade-in">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3.5">
                ...
                {existingImages.map((img) => (
                  <div key={img.id} className="relative aspect-square rounded-[2px] overflow-hidden border border-white/5 group shadow-md bg-white/[0.02]">
                    <img
                    ...
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center">
                      <button
                        ...
                        className="p-2 bg-rose-500/20 hover:bg-rose-500/80 text-rose-300 hover:text-white rounded-[2px] transition duration-200 cursor-pointer border border-rose-500/30"
                        title="Delete image"
                      >
                ...
                {previews.map((url, idx) => (
                  <div key={url} className="relative aspect-square rounded-[2px] overflow-hidden border-2 border-dashed border-[var(--accent-color)]/30 group shadow-md bg-white/[0.02]">
                    <img
                      src={url}
                      alt="Upload Preview"
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                    />
                    <div className="absolute top-1.5 right-1.5 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wider bg-[var(--accent-color)] text-white rounded-[2px] shadow-md">
                      New
                    </div>
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center">
                      <button
          ...
          <div className="flex items-center justify-between pt-5 border-t border-white/5 text-xs text-[#ADA9BA] font-medium">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                type="submit"
                disabled={loading}
                className="bg-[var(--accent-color)] hover:bg-[var(--accent-gradient-from)] text-white font-bold px-6 py-3 rounded-[2px] text-xs transition-all duration-300 flex items-center gap-2 shadow-md group cursor-pointer disabled:opacity-50 tracking-wider uppercase"
              >
                {loading ? (
                  <>
        ...
        <AnimatePresence>
          {!isFocusMode && (
            ...
            >
              {loading ? (
                <div className="p-8 glass-card rounded-[2px] flex flex-col items-center justify-center text-center py-20 shadow-sm animate-pulse border border-white/5">
                  <div className="w-12 h-12 rounded-[2px] bg-[var(--accent-color)]/10 flex items-center justify-center mb-4">
                    <Heart className="w-6 h-6 text-[var(--accent-heart)] fill-[var(--accent-heart)] animate-bounce" />
                  </div>
                  <p className="text-md font-serif font-bold text-[#F3F3F5]">AI Processing</p>
                  <p className="text-xs text-[#ADA9BA] mt-2 max-w-[200px] leading-relaxed">Extracting topics, tags, and emotional tone...</p>
                </div>
              ) : savedData ? (
                <motion.div 
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1 }}
                  className="p-6 glass-card rounded-[2px] space-y-6 shadow-sm border border-emerald-500/10"
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <span className="inline-block px-3 py-1 text-xs font-bold rounded-full capitalize bg-[var(--accent-color)]/10 text-[var(--accent-color)] border border-[var(--accent-color)]/20 shadow-sm">
                      {savedData.mood}
                    </span>
                  </div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {savedData.tags && savedData.tags.length > 0 ? (
                        savedData.tags.map((tg) => (
                          <span key={tg} className="text-xs bg-white/[0.04] hover:bg-white/[0.08] text-[#ADA9BA] hover:text-[#F3F3F5] px-2.5 py-1 rounded-[2px] border border-white/5 transition-all">
                            #{tg}
                          </span>
                        ))
                      ) : (
                  ...
                  <p className="text-xs text-[#ADA9BA] leading-relaxed italic bg-white/[0.02] p-4 rounded-[2px] border border-white/5 font-serif">
                    "This entry has been saved to your memory history."
                  </p>
                </motion.div>
              ) : (
                <div className="p-6 glass-card rounded-[2px] space-y-5 shadow-sm border border-white/5">
                  <div className="flex items-center gap-2 text-[var(--accent-color)]">
                    <Heart className="w-4 h-4 text-[var(--accent-heart)] fill-[var(--accent-heart)]" />
                  ...
                  <p className="text-xs text-[#ADA9BA] leading-relaxed">
                    Saving your entry will analyze the text to map emotional patterns, detect tags, and index it for search.
                  </p>
                  ...
                  <div className="text-xs space-y-3 pt-2 text-[#ADA9BA]">
                    ...
                    <div className="flex justify-between pb-1">
                      ...
                      <span className="text-[var(--accent-color)] font-semibold">Ready</span>
                    </div>
                  </div>
                </div>
              )}
